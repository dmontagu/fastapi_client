...
class AuthMiddleware:
    ...
    async def __call__(self, request: AsyncRequest, call_next: Send) -> AsyncResponse:
        ...
        if tokens:
            self.on_received_tokens(tokens=tokens)
    ...
    def on_received_tokens(self, tokens: TokenSuccessResponse) -> None:
        """
        A hook for interacting with received tokens when auth succeeds.
        """
        return
